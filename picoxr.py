...
def dailyShare():
    url="https://bbs.picoxr.com/ttarch/api/growth/v1/user/share?app_id=264482&service_id=0&lang=zh-Hans-CN"
    headers={
        'Host':'bbs.picoxr.com',
        'Connection':'keep-alive',
        'Accept':'application/json,text/plain,*/*',
        'User-Agent':'Mozilla/5.0(WindowsNT10.0;Win64;x64)AppleWebKit/537.36(KHTML,likeGecko)Chrome/117.0.0.0Safari/537.36',
        'Referer':'https://bbs.picoxr.com/post/7281503888166862905',
        'Cookie': PICOXR_COOKIE,
    }
    response = requests.post(url=url, headers=headers,)
    result = json.loads(response.text)
    print("每日分享任务：",result["data"])
